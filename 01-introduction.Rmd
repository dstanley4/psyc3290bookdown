...
Note: If you are not in the class, the data file is available from: https://github.com/dstanley4/psyc3290bookdown
...
Download the data_ratings.csv file from the Lab 1 folder on Courselink. Then go back to RStudio Cloud. Use the **Upload** button in RStudio Cloud to import the data_ratings.csv file. The button is displayed below. **You need to practice getting data from Courselink and Uploading it to RStudio Cloud because this will be required on the Lab Midterm Exam.**

```{r uploadbutton, fig.cap="Screenshot of upload button", echo = FALSE, out.width="50%"}
knitr::include_graphics("ch_introduction/images/screenshot_upload_button.png")
```

Then repeat the above exercise using the code below:
